...
| D9 | recurring_journal_entries | EXPORT + ASK | Export the recurring-transactions list. Which JEs are standard monthly (prepaid spreads)? Which are deliberately NOT recurring because a skill generates them? |
...
| D21 | sales_deposit_reconciliation_rules | ASK | **Ask what the reconciliation authority IS before asking how it ties.** Per channel: does revenue tie to the *payout* or to a *statement* the platform/manager sends? Where a statement governs, it is the reconciliation — gross-to-net is the wrong frame and the answer will be "the premise is wrong." Then: fees netted where, or grossed back up into their own expense accounts? (Some platforms have no fees — payout ties exactly; say so.) |
| D21b | sales_accrual_and_cutoff | ASK | **The period cut-off — where accrued revenue actually lives on this firm's clients.** Per channel: revenue earned inside the period whose payout or statement lands *after* month end — is it **accrued into the period**, or recognized when it settles? What supports the accrual (platform report, manager statement, batch export), who computes it, and does it post as a month-end JE (cross-reference D9b)? Where does in-transit money sit meanwhile — undeposited funds, a clearing account (cross-reference D8)? Watch for **statement windows that straddle month end** (e.g. a property-manager statement covering the 20th of the prior month through the 11th of the current): deposits are then split in the statement's month, not the month they land, which sets both the accrual and the close's critical path. *Conditional:* gift cards, prepaid tuition, deposits and retainers taken through the same channel — capture the deferred-revenue side too where they exist. "Settles same day, nothing to accrue" is a complete answer — record it explicitly. |
